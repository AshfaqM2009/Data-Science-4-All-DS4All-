    ...
    <div className="space-y-4">
      {sorted.map((table) => {
        ...
        return (
          ...
          >
            <div className="bg-emerald-950/30 px-3 py-2 flex items-center gap-2">
              ...
              {isHighlighted && (
                <span className="ml-auto text-[9px] uppercase tracking-wide text-emerald-400 border border-emerald-600/50 rounded px-1.5 py-0.5">
                  used in this quest
                </span>
              )}
            </div>
            <p className="px-3 pt-2 text-[11px] text-emerald-600">{table.description}</p>

            <div className="p-3 overflow-x-auto">
              <table className="w-full text-[11px] mb-3">
                <thead>
                  <tr className="text-emerald-500 border-b border-emerald-900/50">
                ...
                <tbody>
                  {table.columns.map((col) => (
                    <tr key={col.name} className="border-b border-emerald-950/50">
                      <td className="py-1 pr-3 font-mono text-emerald-300">{col.name}</td>
                      <td className="py-1 pr-3 font-mono text-teal-400">{col.type}</td>
                      <td className="py-1 text-emerald-600">{col.description}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="text-emerald-600 border-b border-emerald-900/50">
                    {table.columns.map((col) => (
                ...
                <tbody>
                  {table.sampleRows.map((row, i) => (
                    <tr key={i} className="border-b border-emerald-950/50">
                      {table.columns.map((col) => (
                        <td key={col.name} className="py-1 pr-3 font-mono text-emerald-400">
                          {String(row[col.name])}
                        </td>
                      ))}
                    </tr>
