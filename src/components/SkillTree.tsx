...
export function SkillTree({
  ...
  return (
    <aside className="w-full lg:w-[340px] flex-shrink-0 border-l border-emerald-900/40 bg-[#0b100e] flex flex-col overflow-y-auto">
      <div className="px-4 py-3 border-b border-emerald-900/40">
        <h2 className="text-sm font-semibold text-emerald-300">Skill Tree &amp; Upgrades</h2>
        <p className="text-[11px] text-emerald-700 mt-0.5">Tycoon progression menu</p>
      </div>

      <section className="px-4 py-3 border-b border-emerald-900/40">
        ...
        <div className="rounded-lg border border-emerald-900/40 bg-emerald-950/30 p-3">
          <p className="text-sm font-mono text-emerald-200">{currentGpu.name}</p>
          ...
          {nextGpu ? (
            <button
              ...
              className="w-full flex items-center justify-between rounded-md border border-emerald-700/50 bg-emerald-900/30 px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-800/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              <span>Upgrade → {nextGpu.name}</span>
              <span className="font-mono">${nextGpu.cost}</span>
            </button>
          ) : (
            <p className="text-[11px] text-emerald-500">Max GPU tier reached 🎉</p>
          )}
        </div>
      </section>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {unlocked ? (
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-[11px] font-mono text-emerald-400 border border-emerald-700/50 rounded px-2 py-1 hover:bg-emerald-800/30 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      <Lock className="w-3 h-3" />${toolkit.cost}
                    </button>
                  )}
                </div>
                <p className="text-[11px] text-emerald-600 mt-1">{toolkit.description}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="space-y-2">
          {automations.map((auto) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-medium text-emerald-200">{auto.name}</span>
                  {unlocked ? (
                    ...
                    <button
                      ...
                      className="text-[11px] font-mono text-emerald-400 border border-emerald-700/50 rounded px-2 py-1 hover:bg-emerald-800/30 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      ${auto.cost}
                    </button>
                  )}
                </div>
                <p className="text-[11px] text-emerald-600 mt-1">{auto.description}</p>
              </div>
            )
          })}
        </div>
